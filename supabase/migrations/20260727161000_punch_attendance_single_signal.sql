-- ══════════════════════════════════════════════════════════════
-- punch_attendance(): honour attendance_policy.allow_single_signal
--
-- MVP decision (2026-07-27, approved in chat): Sunspeed runs
-- GPS-only attendance. There is no office Wi-Fi gateway, so the
-- previous behaviour — every in-range punch parked at 'pending'
-- awaiting a Wi-Fi assertion — meant ~220 manual reviews/month
-- for a 5-person team. Unworkable.
--
-- `allow_single_signal` already existed in attendance_policy as
-- the intended switch for exactly this; the function simply never
-- read it. This migration wires it up:
--
--   outside geofence                    -> blocked   (unchanged)
--   in range, accuracy missing/too poor -> pending   (human review)
--   in range, accuracy OK, single-signal allowed -> verified
--   in range, accuracy OK, single-signal denied  -> pending
--   no GPS at all                       -> pending
--
-- TRUST NOTE: browser GPS is spoofable. Enabling single-signal is
-- a deliberate, documented relaxation for a 6-person site where
-- everyone is co-located daily — NOT a general endorsement. See
-- CLAUDE.md rule 3 and BACKEND-CONTRACT.md rules 4-5, both amended
-- in the same commit. Re-tighten by setting allow_single_signal
-- back to false; no code change needed.
-- ══════════════════════════════════════════════════════════════
...
begin
  ...
  then
    ...
    /* GPS supplied but no office coordinates configured yet. */
    v_status := 'pending';
...
-- ── Turn single-signal verification on for Sunspeed ──────────
do $$
declare
  v_previous boolean;
begin
  select allow_single_signal into v_previous
  from public.attendance_policy where id;

  if v_previous is distinct from true then
    update public.attendance_policy
    set allow_single_signal = true
    where id;
    ...
    insert into public.attendance_audit_events (
      ...
      'attendance_policy',
      'singleton',
      jsonb_build_object('allow_single_signal', v_previous),
      jsonb_build_object('allow_single_signal', true),
      'GPS-only MVP: in-geofence punches with acceptable accuracy now auto-verify. '
      || 'Approved in chat 2026-07-27. No Wi-Fi gateway deployed.'
    );
  end if;
end;
$$;
